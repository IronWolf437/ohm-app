...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
 ...
 """
    def configure(self, **kwargs):
        # Handle frame_border_color
        if "frame_border_color" in kwargs:
            self.frame_border_color = kwargs.pop("frame_border_color")
            self.frame.configure(border_color=self.frame_border_color)  # Apply to the scrollable frame
        ...
        # Apply remaining configurations to the buttons
        for key in self.widgets.keys():
            self.widgets[key].configure(**kwargs)
